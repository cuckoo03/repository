 ...
 */
@TypeChecked
class TopicTokenizer2 extends Tokenizer {
...
//	private final PayloadAttribute payloadAtt = addAttribute(PayloadAttribute.class);
	...
	public TopicTokenizer2(Reader input) {
		super(input);
...
//				def b = buffer.toString().trim()
				...
				if (word.contains(token)) {
					def index = word.indexOf(token)
					word = word.substring(index, word.size())
					println "before buffer:${buffer.toString()}"
					break;
				}
...
//		def bytesRef = new BytesRef(tnos[tokenIndex].getBytes("UTF-8"));
