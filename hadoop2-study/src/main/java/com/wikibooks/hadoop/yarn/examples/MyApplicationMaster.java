...
public class MyApplicationMaster {
	...
	public boolean init(String[] args) throws Exception {
		...
		if (!envs.containsKey(ApplicationConstants.Environment.CONTAINER_ID
				.name())) {
		...
		if (!envs.containsKey(ApplicationConstants.Environment.NM_HTTP_PORT
				.name())) {
			throw new RuntimeException(
					ApplicationConstants.Environment.NM_HTTP_PORT
							+ " not set in the environment");
		}
		if (!envs.containsKey(ApplicationConstants.Environment.NM_PORT.name())) {
			throw new RuntimeException(
					ApplicationConstants.Environment.NM_PORT.name()
							+ " not set in the environment");
		}
		if (envs.containsKey(Constants.AM_JAR_PATH)) {
			...
			if (envs.containsKey(Constants.AM_JAR_TIMESTAMP)) {
				appJarTimestamp = Long.valueOf(envs
						.get(Constants.AM_JAR_TIMESTAMP));
			}
			...
			if (!appJarPath.isEmpty()
					&& (appJarTimestamp <= 0 || appJarPathLen <= 0)) {
				LOG.error("Illegal values in env for shell script path"
						+ ", path=" + appJarPath + ", len=" + appJarPathLen
		...
		if (numTotalContainers == 0) {
			throw new IllegalArgumentException(
					"Cannot run MyAppliCationMaster with no containers");
		}
	...
	public void run() throws Exception {
		...
		while (allocatedContainers < numTotalContainers) {
			...
			for (ContainerStatus status : response
				...
				++completedContainers;
				LOG.info("ContainerID:" + status.getContainerId() + ", state:"
	...
	private LocalResource createAppMasterJar() throws IOException {
		...
		if (!appJarPath.isEmpty()) {
			...
			appMasterJar
					.setResource(ConverterUtils.getYarnUrlFromPath(jarPath));
	...
	private ContainerLaunchContext createContainerLaunchContext(
		...
		ContainerLaunchContext appContainer = Records
		...
		appContainer.setCommands(Collections
				.singletonList("$JAVA_HOME/bin/java" + " -Xmx256M"
