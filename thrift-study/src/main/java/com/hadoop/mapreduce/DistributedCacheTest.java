...
public class DistributedCacheTest {
	static class CodeComparMapper extends
		...
		protected void setup(Context context) throws IOException,
			...
			super.setup(context);
			URI[] codeFiles = DistributedCache.getCacheFiles(context
					.getConfiguration());
	...
	public static void main(String[] args) throws IOException,
		...
		if (args.length != 3) {
			System.err.println("Usage: java DistributedCacheTest "
					+ "<in> <out> <code file(hdfs)");
			System.exit(1);
		}
		Job job = new Job(conf, "WordCount");
		job.setJarByClass(DistributedCacheTest.class);
		job.setMapperClass(CodeComparMapper.class);
