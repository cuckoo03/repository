...
public class BlogClient {
	public static void main(String[] args) throws Exception {
		...
		System.out.println(serverStatus.getHostName() + ":"
				+ serverStatus.getPort() + " status "
				+ serverStatus.getStatus());

		BlogArticle article = new BlogArticle();
		article.setUserId("user01");
		article.setTitle("Blog thrift example");
		article.setContents(ByteBuffer.wrap("Blog thrift example is very easy"
				.getBytes()));
		...
		article.setContents(ByteBuffer.wrap("Hello thrift example is very easy"
				.getBytes()));
