...
public class SearchCrawler extends JFrame {
	...
	private void search(final String logFile, final String startUrl,
		...
		Thread thread = new Thread(new Runnable() { // thread 생성
					public void run() { // thread 시작
						...
						setCursor(Cursor
								.getPredefinedCursor(Cursor.WAIT_CURSOR));// 모래시계
																			// 커서로
																			// 바뀜
						...
						// Switch search button to "Stop." //로봇 활동 중에 search 버튼을
						// stop버튼으로 바꿔줌
						searchButton.setText("Stop");

						// Reset stats.
						table.setModel(new DefaultTableModel(new Object[][] {},// URL아래
																				// Table에
																				// 관한
																				// 세팅
								new String[] { "URL" }) {
							public boolean isCellEditable(int row, int column) {
								// 넘오는 값에 상관없이 테이블에 값이 나타나는 것을 방지 -> 값이 이상하면
								// false
								return false;
							}
						});
						...
						try {
							logFileWriter = new PrintWriter(new FileWriter(
									logFile));
						} catch (Exception e) {
						...
						crawl(startUrl, maxUrls, limitCheckBox.isSelected(), // is
																				// selected는
																				// 체크
																				// 상태를
																				// 확인하는
						...
						// Mark search as done. // 숨겨진 라벨에 활동이 끝났음을 'done'으로 알림
						// ㄴ
						crawlingLabel2.setText("Done");

						// Enable search controls. //사용자가 텍스트 필드를 사용할 수 있도록 활성화
						// 함
						...
						// Switch search button back to "Search." //버튼을 stop ->
						// search로 바꿈
						searchButton.setText("Search");

						// Return to default cursor.
						setCursor(Cursor.getDefaultCursor());// 커서를 모래시계에서 일반
																// 커서로 바꿈
	...
	// 로봇 프로토콜을 이행하기 위한 메서드
	private boolean isRobotAllowed(URL urlToCheck) {
		String host = urlToCheck.getHost().toLowerCase();

		// Retrieve host's disallow list from cache. //로봇 프로토콜에 맞지 않는 것을 담을 리스트
		ArrayList<String> disallowList = (ArrayList<String>) disallowListCache
				.get(host);
	...
	private ArrayList<String> retrieveLinks(URL pageUrl, String pageContents,
		...
		Pattern p = Pattern.compile("<a\\s+href\\s*=\\s*\"?(.*?)[\"|>]", // <a
																			...
																			// 것을
																			// 걸러냄
				Pattern.CASE_INSENSITIVE); // String이 컴파일될때 패턴을 지정 -> JAVA의
											// 정규표현식
		...
		while (m.find()) { // 매치가 되면 계속 돌아간다.
			String link = m.group(1).trim(); // s.substring(m.start(1),
												// m.end(1))와 같은 표현
			...
			if (link.indexOf("://") == -1) { // 이런 형식(://)이 없다면
				...
				if (link.charAt(0) == '/') { // 슬래쉬(/)로 시작하는 절대 url일 경우 (예)
												// /blog/hagi
					link = "http://" + pageUrl.getHost() + link; // 앞에 http://를
																	// 붙인다.
					// Handle relative URLs.
				} else { // 상대 경로
					String file = pageUrl.getFile();
					if (file.indexOf('/') == -1) { /*
													 * index.html과 같은 경우 (슬래쉬가
													 * 완전 없는 경우)
													 */
			 ...
			 * start URL. 나의 로봇 시작 페이지가 http://daum.net이고
	...
	public void crawl(String startUrl, int maxUrls, boolean limitHost, // 로봇이
																		// 본격적으로
																		// 활동
			String searchString, boolean caseSensitive) {
		...
		while (crawling && toCrawlList.size() > 0) {
			...
			if (maxUrls != -1) { // 최대 url갯수가 설정 되어 있다면
				if (crawledList.size() == maxUrls) { // 수집된 리스트의 갯구와 최대 url수가
														// 같을때 중지!!
					break;
				}
			}
			...
			if (!isRobotAllowed(verifiedUrl)) {// 규정에 맞지 않으면 수집을 하지 않고 다음으로 넘어
												// 간다.
				continue;
			}
			...
			// Download the page at the given url. //downloadpage를 호출,수행하여 리턴값을
			// pageContent에 넣음
			 ...
			 */
			if (pageContents != null && pageContents.length() > 0) { // 페이지의 내용이
																		// 있을 경우
				// Retrieve list of valid links from page.
				ArrayList<String> links = retrieveLinks(verifiedUrl,
						pageContents, crawledList, limitHost); // limitHost???
																// 아무데도 설정 되어 있지
																// 않는데 잘 모르겠다
