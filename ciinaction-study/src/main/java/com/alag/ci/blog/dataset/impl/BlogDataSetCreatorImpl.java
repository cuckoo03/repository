...
public class BlogDataSetCreatorImpl implements DataSetCreator {
	...
	protected List<TextDataItem> getBlogTagMagnitudeVectors(
		...
		List<RetrievedBlogEntry> blogEntries = blogQueryResult
				.getRelevantBlogs();
		List<TextDataItem> result = new ArrayList<TextDataItem>();
		InverseDocFreqEstimator freqEstimator = new InverseDocFreqEstimatorImpl(
				blogEntries.size());
		TextAnalyzer textAnalyzer = new LuceneTextAnalyzer(this.tagCache,
				freqEstimator);

		for (RetrievedBlogEntry blogEntry : blogEntries) {
			String text = composeTextForAnalysis(blogEntry);
			TagMagnitudeVector tmv = textAnalyzer
					.createTagMagnitudeVector(text);
	...
	protected BlogAnalysisDataItem getBlogAnalysisDataItem(
			RetrievedBlogEntry blogEntry, TagMagnitudeVector tmv) {
