...
public class WEKAPredictiveBlogDataSetCreatorImpl extends
	...
	private List<TextDataItem> createLearningDataSet(boolean isContinuous)
			throws Exception {
		String[] positiveTags = { "collective intelligence", "data mining",
				"web 2.0" };
		String[] negativeTags = { "child intelligence", "AJAX" };
		return createLearningDataSet(positiveTags, negativeTags, isContinuous);
	}

	private List<TextDataItem> createLearningDataSet(String[] positiveTags,
			String[] negativeTags, boolean isContinuous) throws Exception {
	...
	protected List<TextDataItem> getBlogTagMagnitudeVectors(
			...
			throws IOException {
		List<TextDataItem> tdiList = super
				.getBlogTagMagnitudeVectors(blogQueryResult);
	...
	public Instances createLearningDataSet(String datasetName,
		...
		Instances trainingDataSet = new Instances(datasetName, allAttributes,
				blogEntries.size());
		...
		for (TextDataItem dataItem : blogEntries) {
			Instance instance = createNewInstance(numAttributes,
					trainingDataSet, allTags, dataItem, isContinuous);
	...
	protected FastVector createAttributes(boolean isContinuous) {
		...
		for (Tag tag : allTags) {
			Attribute tagAttribute = createAttribute(tag.getDisplayText(),
					isContinuous);
			allAttributes.addElement(tagAttribute);
		}
		Attribute classificationAttribute = createAttribute(
				"ClassificationAttribute", isContinuous);
	...
	public static void main(String[] args) throws Exception {
		...
		Instances discreteDataSet = dataCreator.createLearningDataSet(
				"nominalBlogData", false);
		System.out.println(discreteDataSet.toSummaryString());
		// Instances continuousDataSet =
		// dataCreator.createLearningDataSet("continuousBlogData",true);
