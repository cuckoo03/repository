...
public class MetaDataVectorImpl implements MetaDataVector {
	...
	/**
	 * Returns the dot product of this vector with another It assumes that the
	 * other vector is normalized
	 * 
	 */
	public double dotProduct(MetaDataVector other) {
		double result = 0.;
		List<TagMagnitude> otherTagMagnitudeList = other
				.getTagMetaDataMagnitude();
	...
	/**
	 * Computes the distance between two vectors. The number is between 0 and
	 * 1.41 The smaller the distance closer the two vectors are to each other.
	 */
	public double distance(MetaDataVector other) {
		double result = 0.;
		Map<Long, TagMagnitude> localMap = new HashMap<Long, TagMagnitude>(
				this.tagMagnitudeMap);
		List<TagMagnitude> otherTagMagnitudeList = other
				.getTagMetaDataMagnitude();
		for (TagMagnitude otm : otherTagMagnitudeList) {
			TagMagnitude tm = this.tagMagnitudeMap.get(otm.getTagId());
	...
	/**
	 * Returns a new normalized vector that is the addition of this and the
	 * other passed in vector
	 * 
	 * @param other
	 *            the other passed in vector
	...
	public MetaDataVector add(MetaDataVector other, double otherScale) {
		...
		for (Long tagId : this.tagMagnitudeMap.keySet()) {
			...
			if (otherTm == null) {
				...
				otherTm.setMagnitude(otherTm.getMagnitude()
						+ thisTm.getMagnitude());
			}
		}

		List<TagMagnitude> resultList = new ArrayList<TagMagnitude>(
				mergeMap.values());
