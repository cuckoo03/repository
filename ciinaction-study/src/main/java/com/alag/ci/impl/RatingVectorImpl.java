...
public class RatingVectorImpl implements RatingVector {
	...
	public RatingVectorImpl(OwnerType ownerType,
			List<RatingMagnitude> magnitudes) {
		...
		if (other != null) {
			...
			for (Long id : otherMap.keySet()) {
				...
				if (ratingMagnitudeSelf != null) {
					...
					mergedMap.put(id, new TwoRatingMagnitudes(
							ratingMagnitudeSelf, otherRatingMagnitude));
				}
			}
			double sumDot = 0.;
			double sumSelfSqd = 0.;
			...
			for (TwoRatingMagnitudes trm : mergedMap.values()) {
				...
				sumDot += (selfMagn - selfRatingMagnitude
						.getOwnerRatingAverage())
						* (otherMagn - otherRatingMagnitude
								.getOwnerRatingAverage());
				sumSelfSqd += (selfMagn - selfRatingMagnitude
						.getOwnerRatingAverage())
						* (selfMagn - selfRatingMagnitude
								.getOwnerRatingAverage());
	...
	public double getCorrelation(RatingVector other) {
		...
		if (other != null) {
			...
			for (TwoRatingMagnitudes trm : mergedMap.values()) {
				...
				sumSelfSqd += (selfMagn - selfAverage)
						* (selfMagn - selfAverage);
				sumOtherSqd += (otherMagn - otherAverage)
						* (otherMagn - otherAverage);
			}
			// Check to make sure denominator is not zero
			if (sumSelfSqd * sumOtherSqd > 0.) {
