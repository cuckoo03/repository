...
public class KNNWEKAExample {
	...
	private Instances createLearningDataSet(FastVector allAttributes) {
		Instances trainingDataSet = new Instances("wekaCF", allAttributes, 3);
	...
	private void addInstance(Instances trainingDataSet, double item1,
			double item2, double item3) {
	...
	public void illustrateClassification(Instances instances) throws Exception {
		...
		for (int i = 0; i < instances.numInstances(); i++) {
			...
			System.out.println("Expected=" + instance.value(2) + " Predicted="
					+ result);
		}
	}
}
