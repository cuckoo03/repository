...
public class NaiveCrawlerTest extends TestCase {
	...
	private static final String WIKIPEDIA_CI_URL = "http://www.wikipedia.org/wiki/Collective+intelligence";
	...
	private static final String REGEXP_RELATIVE = "<a href=\"/((\\w)+(/))?(\\w)+(.html)?(.htm)?(/[\\w\\.\"]+)?";

	// "<a href=\"/((\\w)+(/))+(\\w)+(.html)?(/[&?               \\+\\%/\\.\\w]+)?(\">)?";
	private static final String REL_URL1 = "<a href=\"/links/index.html#soon\">";
	private static final String REL_URL2 = "<a href=\"/catalog/java\">";
	private static final String REL_URL3 = "<a href=\"/academic\">";

	private NaiveCrawler getCrawler(String seedUrl, String regexp)
			throws Exception {
	...
	public void testAreWeAllowedToVisit() throws Exception {
		...
		assertFalse("We should not be able to crawl " + WIKIPEDIA_URL
				+ "/trap/", flag);
	...
	public void testContentParsing() {
		...
		assertTrue(s1 + " not parsed correctly " + list,
				isStringPresent(url, list));
