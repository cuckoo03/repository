...
public class BlogSearchExample {
	...
	public Directory createSearchIndex(String path,
		...
		System.out
				.println("Number of docs indexed = " + indexWriter.docCount());
	...
	private void addField(Document document, String fieldName, String value,
			Field.Store filedStore, Field.Index fieldIndex,
			Field.TermVector fieldTermVector) {
		Field field = new Field(fieldName, getNotNullValue(value), filedStore,
				fieldIndex, fieldTermVector);
		document.add(field);
	}
	...
	public void searchForBlogs(Directory directory, String queryString)
		...
		System.out.println("Number of results = " + hits.length() + " for "
				+ queryString);
		...
		while (iterator.hasNext()) {
			...
			Explanation explanation = indexSearcher
					.explain(weight, hit.getId());
