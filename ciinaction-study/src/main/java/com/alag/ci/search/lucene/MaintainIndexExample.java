...
public class MaintainIndexExample extends BlogSearchExample {
	...
	public void illustrateBatchModifications(Directory indexDirectory,
			List<Term> deletionTerms, List<Document> addDocuments)
			throws Exception {
		IndexReader indexReader = IndexReader.open(indexDirectory);
	...
	public void illustrateFilterSearch(IndexSearcher indexSearcher,
		...
		Filter rangeFilter = new RangeFilter("modifiedDate", "20080101",
				"20080131", true, true);
		CachingWrapperFilter cachedFilter = new CachingWrapperFilter(
				rangeFilter);
	...
	public void illustrateTermFreqVector(Directory indexDirectory)
		...
		for (int i = 0; i < indexReader.numDocs(); i++) {
			...
			TermFreqVector termFreqVector = indexReader.getTermFreqVector(i,
					"completeText");
	...
	public void illustrateMoreLikeThisByQuery(Directory indexDirectory)
		...
		for (int i = 0; i < indexSearcher.maxDoc(); i++) {
			...
			System.out.println(document.get("title") + " "
					+ document.get("url"));
			if (document != null) {
				List<RetrievedBlogEntry> relatedBlogs = getRelatedBlogsUsingLuceneMoreLikeThis(
						indexSearcher, i);
	...
	private List<RetrievedBlogEntry> getRelatedBlogEntries(
			IndexSearcher indexSearcher, int docNum) throws Exception {
		...
		Query booleanCompleteTextQuery = composeTermVectorBooleanQuery(
				indexReader, docNum, "completeText", 1.0f);
	...
	private List<RetrievedBlogEntry> getRelatedBlogEntries(
			IndexSearcher indexSearcher, Query query) throws Exception {
		RetrievedBlogHitCollector blogHitCollector = new RetrievedBlogHitCollector(
				indexSearcher);
	...
	private Query composeTermVectorBooleanQuery(IndexReader indexReader,
		...
		TermFreqVector termFreqVector = indexReader.getTermFreqVector(docNum,
				fieldName);
	...
	public List<RetrievedBlogEntry> getRelatedBlogsUsingLuceneMoreLikeThis(
			IndexSearcher indexSearcher, int docNum) throws Exception {
		IndexReader indexReader = indexSearcher.getIndexReader();
	...
	public static void main(String[] args) throws Exception {
		...
		IndexReader indexReader = IndexReader.open(indexDirectory);
		Query query = bs.composeTermVectorBooleanQuery(indexReader, 1,
				"completeText", 1);
		// System.out.println("\n\nQuery = " + query);
